...
(:action goto-cr
	...
	:precondition (and (set ?oi2) (set ?oi1) (sent-to-human ?b))
	:effect (and 
		(not (set ?oi2))
	)
)
...
(:action goto-rirc1
	:parameters (?oi - oi ?w1 - w1 ?w2 - w2 ?i - i ?iq - iq)
	:precondition (and (sent-to-human ?i) (sent-to-robot ?iq) (sent-to-human ?iq))
	:effect (and 
		(sent-to-robot ?i)
	)
)
...
(:action goto-cir
	:parameters (?oi1 - oi1 ?oi2 - oi2 ?b - b)
	:precondition (and (sent-to-robot ?b) (set ?oi2) (sent-to-human ?b))
	:effect (and 
		(not (sent-to-robot ?b))
		(set ?oi1)
		(not (sent-to-human ?b))
